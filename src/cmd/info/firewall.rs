...
pub fn print_firewall_status() -> anyhow::Result<()> {
    ...
    let ufw_activity = match ufw_output {
        Ok(_) => { has_ufw = true; "active".green().bold() },
        Err(_) => "inactive".red().bold()
    };

    let firewalld_output = Command::new("firewall-cmd")
    .arg("--state")
    .output();
    let firewalld_activity = match firewalld_output {
        Ok(_) => { has_firwalld = true; "active".green().bold() },
        Err(_) => "inactive".red().bold()
    };
    ...
    if !has_ufw && !has_firwalld {
        let output = format!("{}", 
        "No active firewall detected. Services may be exposed to public.".color(colors::TEXT_DEFAULT));
        print::println("");
